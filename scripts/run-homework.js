...
const { homework } = require("./manifest");
...
const scriptPath = homework[scriptRequested];

if (!scriptPath) {
  console.error(
    `${chalk.bold.bgRed(" ERROR ")} Homework "${chalk.bold(
      scriptRequested
    )}" found`
  );

  console.log(
    `\n${chalk.bold("The Available Homework questions are:")}${getScriptList(
      homework
    )}\n\n`
  );
